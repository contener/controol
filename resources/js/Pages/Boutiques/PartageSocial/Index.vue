...
const statutClasses = {
    en_attente: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300',
...
<template>
    <AppLayout title="Partage & publication sociale">
        <template #header>
            <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-100 leading-tight">Partage &amp; publication sociale</h2>
        </template>

        <div class="py-8">
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div v-if="!autorise" class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg p-6">
                    <p class="font-medium text-slate-900 dark:text-slate-100">Publication sociale indisponible</p>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Votre abonnement actuel ne permet pas cette fonctionnalité.</p>
                ...
                <template v-else>
                    ...
                    <div v-if="campagneEnCours" class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300">Publication en cours...</h3>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                <div class="h-full bg-blue-600 transition-all" :style="`width: ${(destinationsTraitees / campagne.campagne_destinations.length) * 100}%`" />
                            </div>
                        </div>

                        <div v-if="prochaineDestination" class="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                            <p class="text-sm font-medium text-slate-900 dark:text-slate-100">Prochaine destination : {{ prochaineDestination.destination.nom }}</p>

                            <div class="mt-3 bg-slate-50 dark:bg-slate-900 rounded-md p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-line">{{ messagePourDestination }}</div>
                            ...
                            <p class="mt-4 text-xs text-slate-500 dark:text-slate-400">Une fois le message publié à la main sur cette destination, confirmez le résultat :</p>
                    ...
                    <div v-else class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Message à publier</h3>

                        <textarea
                            v-model="formCampagne.message"
                            rows="5"
                            class="block w-full border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm text-sm"
                        />
                        ...
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            <button
                                ...
                                :key="variable.code"
                                type="button"
                                class="px-2 py-1 text-xs font-mono rounded border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                        ...
                        <div class="mt-3">
                            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Aperçu :</p>
                            <div class="bg-slate-50 dark:bg-slate-900 rounded-md p-3 text-sm text-slate-700 dark:text-slate-300 whitespace-pre-line">{{ apercuMessage }}</div>
                        </div>

                        <div class="mt-4 border-t border-slate-200 dark:border-slate-700 pt-4">
                            <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">⚙ Paramètres de publication</h4>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-slate-500 dark:text-slate-400">secondes</span>
                            </div>
                        ...
                        <p v-if="destinations.length === 0" class="mt-2 text-xs text-slate-400 dark:text-slate-500 text-right">Ajoutez au moins une destination ci-dessous.</p>
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg">
                        <div class="p-6 flex items-center justify-between">
                            <div>
                                <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300">Groupes / Destinations de publication</h3>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-1" v-if="limiteDestinations !== null">{{ destinations.length }} / {{ limiteDestinations }} destinations utilisées</p>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                                <thead class="bg-slate-50 dark:bg-slate-900">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Nom du groupe</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Lien</th>
                                ...
                                <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                                    <tr v-if="destinations.length === 0">
                                        <td colspan="4" class="px-6 py-6 text-center text-slate-400 dark:text-slate-500">Aucune destination enregistrée.</td>
                                    </tr>
                                    <tr v-for="destination in destinations" :key="destination.id" class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                        <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-slate-900 dark:text-slate-100">{{ destination.nom }}</td>
                                        <td class="px-6 py-3 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400 max-w-xs truncate">
                                        ...
                                        <td class="px-6 py-3 whitespace-nowrap text-right text-sm space-x-3">
                                            <button class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300" @click="ouvrirEditionDestination(destination)">Modifier</button>
