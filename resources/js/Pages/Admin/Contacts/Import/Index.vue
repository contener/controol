    ...
    <AppLayout title="Importer des contacts">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-100 leading-tight">Importer des contacts</h2>
                <Link :href="route('admin.contacts.index')" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">← Retour aux contacts</Link>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div v-if="etape === 'upload'" class="bg-white dark:bg-slate-800 shadow-sm rounded-lg p-6">
                    <InputLabel value="Fichier Excel ou CSV" />
                    <input type="file" accept=".xlsx,.xls,.csv" class="mt-2 block w-full text-sm text-slate-600 dark:text-slate-300" @change="selectionnerFichier" />
                ...
                <template v-if="etape === 'apercu' && analyse">
                    <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg p-6">
                        ...
                        <div class="mt-4 text-sm text-slate-500 dark:text-slate-400 space-y-1">
                            <p>
                                <span class="font-medium text-slate-700 dark:text-slate-300">Colonnes téléphone détectées :</span>
                            ...
                            <p v-if="analyse.colonne_whatsapp_detectee">
                                <span class="font-medium text-slate-700 dark:text-slate-300">Colonne WhatsApp détectée :</span>
                                {{ analyse.colonne_whatsapp_detectee }}
                            </p>
                            <p>
                                <span class="font-medium text-slate-700 dark:text-slate-300">Colonnes e-mail détectées :</span>
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                            <thead class="bg-slate-50 dark:bg-slate-900">
                                <tr>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Ignorer</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Nom</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Téléphone</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">WhatsApp</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Numéro normalisé</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Statut</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700 text-sm">
                                <tr v-for="ligne in analyse.apercu" :key="ligne.index" :class="lignesIgnorees.has(ligne.index) ? 'opacity-40' : ''">
                                    ...
                                    <td class="px-4 py-2">
                                        {{ ligne.telephone ?? '—' }}
                                        <span v-if="ligne.numeros_secondaires > 0" class="text-xs text-slate-400">(+{{ ligne.numeros_secondaires }} autre{{ ligne.numeros_secondaires > 1 ? 's' : '' }})</span>
                                    </td>
                        ...
                        <p v-if="analyse.total_lignes > analyse.apercu.length" class="px-4 py-3 text-xs text-slate-400 dark:text-slate-500">
                ...
                <div class="bg-white dark:bg-slate-800 shadow-sm rounded-lg">
                    <div class="p-6 pb-3">
                        <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300">Imports précédents</h3>
                    </div>
                    <div v-if="imports.data.length === 0" class="px-6 pb-6 text-sm text-slate-400 dark:text-slate-500">Aucun import pour le moment.</div>
                    <div v-else class="divide-y divide-slate-100 dark:divide-slate-700">
                        <Link v-for="imp in imports.data" :key="imp.id" :href="route('admin.contacts.import.show', imp.id)" class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700">
                            <div>
                                <div class="font-medium text-slate-900 dark:text-slate-100">{{ imp.nom_fichier }}</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                    par {{ imp.admin?.name ?? '—' }} le {{ formatDateHeure(imp.created_at) }} — {{ statutLabels[imp.statut] }}
                                </div>
                            </div>
                            <div class="text-sm text-slate-500 dark:text-slate-400">{{ imp.lignes_importees }} créés · {{ imp.lignes_maj }} maj</div>
                        </Link>
                    </div>
                </div>
            </div>
