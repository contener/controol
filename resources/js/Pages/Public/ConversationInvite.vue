    ...
    <div class="min-h-screen bg-slate-50">
        <header class="bg-white border-b border-slate-100">
            <div class="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
                ...
                <Link :href="route('marketplace.index')" class="text-sm font-medium text-slate-600 hover:text-slate-900">Marketplace</Link>
            </div>
        </header>
        ...
        <div class="max-w-2xl mx-auto px-4 py-8">
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h1 class="font-semibold text-slate-900">{{ conversation.boutique.nom }}</h1>
                        <p v-if="conversation.produit" class="text-xs text-slate-500">À propos de « {{ conversation.produit.nom }} »</p>
                    </div>
                </div>

                <ConversationThread :messages="conversation.messages" me="visiteur" class="max-h-96 overflow-y-auto" />

                <div v-if="conversation.statut === 'bloquee'" class="mt-4 text-sm text-slate-400 text-center border-t border-slate-100 pt-4">
                    Cette conversation n'accepte plus de nouveaux messages.
                </div>
                <div v-else class="mt-4 border-t border-slate-100 pt-4">
                    <textarea v-model="form.contenu" rows="3" class="block w-full border-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm text-sm" placeholder="Écrire une réponse..." />
