    ...
    <AppLayout title="Dépenses">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">Dépenses</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Catégorie</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-if="depenses.data.length === 0">
                                <td colspan="5" class="px-6 py-6 text-center text-gray-400 dark:text-gray-500">Aucune dépense enregistrée.</td>
                            </tr>
                            <tr v-for="depense in depenses.data" :key="depense.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ depense.date_depense }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ depense.categorie }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ depense.description }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">{{ formatMontant(depense.montant) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-3">
                                    <Link :href="route('depenses.edit', depense.id)" class="text-indigo-600 hover:text-indigo-900 dark:hover:text-gray-200">Modifier</Link>
                                    <button class="text-red-600 hover:text-red-900 dark:hover:text-gray-200" @click="supprimer(depense)">Supprimer</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
