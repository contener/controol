...
<template>
    <div class="bg-white text-slate-900 text-sm print:p-0">
        ...
        <div class="p-8 print:p-0 print:pt-6">
            <div class="flex justify-end gap-8 text-xs text-slate-500 mb-6">
                <div>Date d'émission : <span class="font-medium text-slate-700">{{ apercu.meta.date_emission || '—' }}</span></div>
                <div v-if="apercu.meta.date_echeance">Date d'échéance : <span class="font-medium text-slate-700">{{ apercu.meta.date_echeance }}</span></div>
            </div>

            <!-- Émetteur / Client : informations clairement séparées -->
            <div class="grid grid-cols-2 gap-4">
                <div class="border border-slate-300 rounded-md p-4">
                    <div class="text-xs font-semibold text-slate-500 uppercase mb-2 pb-1 border-b border-slate-200">Émetteur</div>
                    <div class="font-medium">{{ apercu.boutique.nom || 'Ma boutique' }}</div>
                    <div v-if="apercu.boutique.adresse || apercu.boutique.ville" class="text-slate-600 mt-1.5 text-xs">
                        <span>📍</span>
                    ...
                    <div class="text-slate-500 mt-1.5 space-y-0.5">
                ...
                <div class="border border-slate-300 rounded-md p-4">
                    <div class="text-xs font-semibold text-slate-500 uppercase mb-2 pb-1 border-b border-slate-200">Client</div>
            ...
            <table class="w-full mt-6 border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-100 uppercase text-slate-500 border border-slate-300">
                        <th class="text-left py-2 px-2 border border-slate-300">Désignation</th>
                        <th class="text-right py-2 px-2 border border-slate-300">Qté</th>
                        <th class="text-right py-2 px-2 border border-slate-300">P.U. HT</th>
                        <th class="text-right py-2 px-2 border border-slate-300">Remise</th>
                        <th class="text-right py-2 px-2 border border-slate-300">TVA %</th>
                        <th class="text-right py-2 px-2 border border-slate-300">Montant HT</th>
                        <th class="text-right py-2 px-2 border border-slate-300">Total TTC</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(ligne, i) in apercu.lignes" :key="i" :class="i % 2 === 1 ? 'bg-slate-50' : ''">
                        ...
                        <td class="py-2 px-2 text-right border border-slate-200">{{ ligne.quantite }}</td>
                        <td class="py-2 px-2 text-right border border-slate-200">{{ formatMontant(ligne.prix_unitaire) }}</td>
                        <td class="py-2 px-2 text-right border border-slate-200">{{ formatMontant(ligne.remise_ligne) }}</td>
                        <td class="py-2 px-2 text-right border border-slate-200">{{ ligne.tva_taux }}</td>
                        <td class="py-2 px-2 text-right border border-slate-200">{{ formatMontant(ligne.montant_ht) }}</td>
                        <td class="py-2 px-2 text-right border border-slate-200">{{ formatMontant(ligne.montant_ttc) }}</td>
                    </tr>
                    <tr v-if="apercu.lignes.length === 0">
                        <td colspan="7" class="py-6 text-center text-slate-400 border border-slate-200">Aucune ligne pour le moment.</td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="mt-8 border-t border-slate-200 pt-4 text-xs text-slate-500">
                <div class="font-semibold text-slate-600 uppercase mb-1">Conditions générales</div>
                <p>Facture payable sous 30 jours. Tout retard de paiement pourra entraîner des pénalités.</p>
            </div>

            <!-- Coordonnées de paiement -->
            <div class="mt-4 border border-slate-200 rounded-md p-4">
                <div class="text-xs font-semibold text-slate-500 uppercase mb-1">Coordonnées de paiement</div>
            ...
            <div class="mt-10 pt-4 border-t border-slate-200 flex justify-between text-xs text-slate-400">
