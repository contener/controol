...
<template>
    <AppLayout title="Stock">
        <template #header>
            <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-100 leading-tight">Gestion des stocks</h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
                <div class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg p-4">
                    <label class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300">Afficher uniquement les produits en rupture de stock</span>
                    </label>
                </div>
                ...
                <div class="bg-white dark:bg-slate-800 shadow-sm sm:rounded-lg overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead class="bg-slate-50 dark:bg-slate-900">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Produit</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Référence</th>
                                <th class="px-6 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Quantité en stock</th>
                                <th class="px-6 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Seuil d'alerte</th>
                        ...
                        <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                            <tr v-if="produits.data.length === 0">
                                <td colspan="5" class="px-6 py-6 text-center text-slate-400 dark:text-slate-500">Aucun produit avec suivi de stock.</td>
                            </tr>
                            <tr v-for="produit in produits.data" :key="produit.id" class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{{ produit.reference ?? '—' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right" :class="produit.seuil_alerte !== null && produit.quantite_stock <= produit.seuil_alerte ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-slate-900 dark:text-slate-100'">
                                    {{ produit.quantite_stock }} {{ produit.unite }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-slate-500 dark:text-slate-400">{{ produit.seuil_alerte ?? '—' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-3">
                                    <button class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300" @click="ouvrirModal(produit)">Mouvement</button>
                                    <Link :href="route('stock.mouvements', produit.id)" class="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-200">Historique</Link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <Modal :show="showModal" @close="fermerModal">
            <div class="p-6">
                <h3 class="text-lg font-medium text-slate-900 dark:text-slate-100">Mouvement de stock — {{ produitCible?.nom }}</h3>
