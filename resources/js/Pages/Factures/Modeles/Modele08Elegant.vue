<script setup>
import { useCurrencyFormat } from '@/Composables/useCurrencyFormat';

const props = defineProps({
    apercu: Object,
});

const { formatMontant } = useCurrencyFormat();
</script>

<template>
    <div class="bg-white text-slate-900 text-sm px-16 py-14 print:p-0 font-light border border-slate-100">
        <div class="text-center">
            <img v-if="apercu.boutique.logo_url" :src="apercu.boutique.logo_url" class="h-12 w-auto object-contain mx-auto mb-5" alt="Logo">
            <h1 class="font-serif text-3xl tracking-[0.15em] uppercase">{{ apercu.boutique.nom || 'Ma boutique' }}</h1>
            <div v-if="apercu.boutique.adresse || apercu.boutique.ville" class="text-slate-400 text-xs mt-3 tracking-wide">
                <span>📍</span>
                <span v-if="apercu.boutique.adresse">{{ apercu.boutique.adresse }}<span v-if="apercu.boutique.ville">, </span></span><span v-if="apercu.boutique.ville">{{ apercu.boutique.ville }}</span><span v-if="apercu.boutique.pays">, {{ apercu.boutique.pays }}</span>
            </div>
            <div class="text-slate-400 text-xs mt-1.5 tracking-wide space-x-2">
                <span v-if="apercu.boutique.email">{{ apercu.boutique.email }}</span>
                <span v-if="apercu.boutique.nui">· NUI {{ apercu.boutique.nui }}</span>
            </div>
        </div>

        <div class="w-10 h-px bg-slate-300 mx-auto my-10"></div>

        <div class="flex justify-between items-start text-xs">
            <div>
                <div class="uppercase tracking-widest text-slate-400 mb-2">Facturé à</div>
                <div class="text-base font-normal">{{ apercu.client.nom }}</div>
                <div class="text-slate-400 mt-1 space-y-0.5">
                    <div v-if="apercu.client.adresse">{{ apercu.client.adresse }}</div>
                    <div v-if="apercu.client.ville">{{ apercu.client.ville }} {{ apercu.client.pays }}</div>
                    <div v-if="apercu.client.email">{{ apercu.client.email }}</div>
                </div>
            </div>
            <div class="text-right">
                <div class="uppercase tracking-widest text-slate-400 mb-2">Facture</div>
                <div class="text-base font-normal">{{ apercu.meta.numero }}</div>
                <div class="text-slate-400 mt-1 space-y-0.5">
                    <div>{{ apercu.meta.date_emission || '—' }}</div>
                    <div v-if="apercu.meta.date_echeance">Échéance {{ apercu.meta.date_echeance }}</div>
                </div>
            </div>
        </div>

        <table class="w-full mt-14 border-collapse">
            <thead>
                <tr class="border-b border-slate-300 text-xs uppercase tracking-widest text-slate-400">
                    <th class="text-left py-3 font-normal">Désignation</th>
                    <th class="text-right py-3 font-normal">Qté</th>
                    <th class="text-right py-3 font-normal">Prix</th>
                    <th class="text-right py-3 font-normal">Total</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(ligne, i) in apercu.lignes" :key="i" class="border-b border-slate-100">
                    <td class="py-4">
                        <div>{{ ligne.designation || '—' }}</div>
                        <div v-if="ligne.description" class="text-xs text-slate-400 mt-0.5">{{ ligne.description }}</div>
                    </td>
                    <td class="py-4 text-right text-slate-500">{{ ligne.quantite }}</td>
                    <td class="py-4 text-right text-slate-500">{{ formatMontant(ligne.prix_unitaire) }}</td>
                    <td class="py-4 text-right">{{ formatMontant(ligne.montant_ttc) }}</td>
                </tr>
                <tr v-if="apercu.lignes.length === 0">
                    <td colspan="4" class="py-10 text-center text-slate-300">Aucune ligne pour le moment.</td>
                </tr>
            </tbody>
        </table>

        <div class="mt-16 pt-10 border-t border-slate-100 text-center">
            <div class="uppercase tracking-widest text-slate-400 text-xs mb-3">Total à payer</div>
            <div class="font-serif text-5xl">{{ formatMontant(apercu.totaux.total_ttc, apercu.meta.devise) }}</div>
            <div class="w-10 h-px bg-slate-300 mx-auto mt-4 mb-3"></div>
            <div class="text-slate-400 text-xs space-x-4">
                <span>Sous-total {{ formatMontant(apercu.totaux.sous_total, apercu.meta.devise) }}</span>
                <span>TVA {{ formatMontant(apercu.totaux.total_tva, apercu.meta.devise) }}</span>
                <span v-if="apercu.totaux.remise">Remise - {{ formatMontant(apercu.totaux.remise, apercu.meta.devise) }}</span>
            </div>
        </div>

        <div v-if="apercu.notes" class="mt-14 text-center text-slate-400 text-xs">
            <p class="whitespace-pre-line">{{ apercu.notes }}</p>
        </div>

        <div v-if="apercu.garantie" class="mt-10 pt-6 border-t border-slate-100 text-center text-slate-400 text-[11px] italic tracking-wide">
            <span class="font-semibold not-italic">Garantie : </span>{{ apercu.garantie }}
        </div>
        <div v-if="apercu.boutique.note_pied_facture" class="mt-3 pt-3 text-center text-slate-400 text-[11px] italic tracking-wide" :class="!apercu.garantie ? 'border-t border-slate-100 pt-6 mt-10' : ''">
            {{ apercu.boutique.note_pied_facture }}
        </div>

        <div class="mt-20 flex justify-between items-end text-xs text-slate-400">
            <div class="tracking-wide">Merci de votre confiance.</div>
            <div class="text-center">
                <div class="w-36 border-b border-slate-200 mb-1">&nbsp;</div>
                <span class="tracking-widest uppercase">Signature</span>
            </div>
        </div>
    </div>
</template>
