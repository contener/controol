    ...
    <AppLayout title="Produits & Services">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-100 leading-tight">Produits &amp; Services</h2>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                            ...
                            <tr v-for="produit in produits.data" :key="produit.id" class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-slate-900 dark:text-slate-100">{{ formatMontant(produit.prix_vente) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                    <span v-if="produit.gere_stock" :class="produit.seuil_alerte !== null && produit.quantite_stock <= produit.seuil_alerte ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-slate-900 dark:text-slate-100'">
