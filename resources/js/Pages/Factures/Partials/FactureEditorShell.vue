    ...
    <div>
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                <span>Modèle : <strong class="text-gray-900 dark:text-gray-100">{{ modeleActuel?.label ?? '—' }}</strong></span>
        ...
        <!-- Onglets mobile -->
        <div class="flex lg:hidden mb-4 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <button
                type="button"
                class="flex-1 py-2 text-sm font-medium"
                :class="ongletMobile === 'saisie' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'"
            ...
            <button
                type="button"
                class="flex-1 py-2 text-sm font-medium"
                :class="ongletMobile === 'apercu' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="lg:sticky lg:top-4 self-start" :class="ongletMobile === 'saisie' ? 'hidden lg:block' : ''">
                <div class="bg-gray-100 dark:bg-gray-950 rounded-lg p-4 shadow-sm max-h-[80vh] overflow-y-auto">
        ...
        <!-- Sélecteur de modèle -->
        <div v-if="pickerOuvert" class="fixed inset-0 z-40 bg-gray-900/60 flex items-start justify-center p-4 overflow-y-auto" @click.self="pickerOuvert = false">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-6xl w-full mt-8 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Choisir un modèle</h3>
                    <button type="button" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" @click="pickerOuvert = false">✕</button>
                </div>
        ...
        <div v-if="pleinEcranOuvert" class="fixed inset-0 z-50 bg-gray-900/80 flex items-start justify-center p-4 overflow-y-auto print:p-0 print:bg-white">
